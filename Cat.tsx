...
import {
  getCats, // Ubah API sesuai dengan file catApi.ts
  addCat, // Ubah API sesuai dengan file catApi.ts
  updateCat, // Ubah API sesuai dengan file catApi.ts
  deleteCat, // Ubah API sesuai dengan file catApi.ts
  // sendCatToTitipan,
} from '../Api/catApi'; // Pastikan path ke catApi.ts sesuai
...
const CatScreen = () => {
  ...
  const fetchCats = async () => {
    try {
      const response = await getCats(); // Mengambil data Cat
      setCatList(response);
    } catch (error) {
      console.error('Error fetching cats:', error);
    }
  ...
  const handleCreateCat = async () => {
    try {
      const response = await addCat(newAnimal);
      setCatList(prevCatList => [...prevCatList, response]); // Menggunakan prevCatList untuk update state dengan benar
      setModalVisible(false);
      setNewAnimal({name: '', type: 'Cat', owner: '', dateTaken: ''});
    } catch (error) {
      console.error('Error creating cat:', error);
  ...
  const handleUpdateCat = async () => {
    if (editingCat) {
      try {
        ...
        setCatList(
          prevCatList =>
            prevCatList.map(cat => (cat.id === editingCat.id ? response : cat)), // Menggunakan prevCatList untuk update state dengan benar
        );
        setModalVisible(false);
        setNewAnimal({name: '', type: 'Cat', owner: '', dateTaken: ''});
        setEditingCat(null);
  ...
  const handleDeleteCat = async (id: number) => {
    try {
      await deleteCat(id);
      setCatList(prevCatList => prevCatList.filter(cat => cat.id !== id)); // Menggunakan prevCatList untuk update state dengan benar
    } catch (error) {
      console.error('Error deleting cat:', error);
    }
  };
      ...
      {catList.length === 0 ? (
        <View style={styles.noDataContainer}>
          <Image
            ...
            style={styles.noDataImage}
          />
          <Text style={styles.noDataText}>Belum ada data Cat</Text>
        </View>
      ) : (
        <FlatList
          data={catList}
...
// Salin styles dari DogScreen, tanpa perubahan
